...
def dem_conditioning(dem, folder, gdb, threshold, show, epsg, drain_network=None):
    ...
    arcpy.ProjectRaster_management(os.path.join(folder, 'temp\dem_temp.tif'), os.path.join(folder, 'temp\work_dem.tif'), epsg, 'BILINEAR', '#', '#', '#', '#')
    ...
    if drain_network:
        ...
        # TODO: The drainage network must be a feature line NOT polygons. Another option is to burn streams by rasterizing the hydrography and doing maps algebra
        ...
        ArcHydroTools.FillSinks(Input_DEM_Raster=os.path.join(gdb_path, r'AgreeDEM'), Output_Hydro_DEM_Raster=os.path.join(gdb_path, r'fill'))

    gp.SetProgressorPosition(20)
    gp.AddMessage('Processing Flow Direction ...')
    ArcHydroTools.FlowDirection(Input_Hydro_DEM_Raster=os.path.join(gdb_path, r'fill'), Output_Flow_Direction_Raster=os.path.join(gdb_path, r'fdr'))
    ...
    ArcHydroTools.StreamSegmentation(os.path.join(gdb_path, r'str'), os.path.join(gdb_path, 'fdr'), os.path.join(gdb_path, 'strlnk'))

    gp.SetProgressorPosition(60)
    gp.AddMessage('Processing Catchment Grid ...')
    ArcHydroTools.CatchmentGridDelineation(os.path.join(gdb_path, r'fdr'), os.path.join(gdb_path, 'strlnk'), os.path.join(gdb_path, 'cat'))
    ...
    ArcHydroTools.DrainagePointProcessing(os.path.join(gdb_path, r'fac'), os.path.join(gdb_path, 'cat'), os.path.join(gdb_path, 'catchment'), os.path.join(gdb_path, 'drainage_point'))
