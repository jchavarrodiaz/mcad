...
def knickpoints_extract(raw_dem, shape_out, drain_network, folder, eq, gdb, epsg):
    ...
    try:
        ...
        msg_error = 'It was not possible to load the existing 2D drainage network'
        gp.SetProgressor('default', 'creating a 3D drain network from based on 2D drainage network and DEM ...')
        drain_2D = drain_network

        ArcHydroTools.Construct3DLine(in_line2d_features=drain_2D, in_rawdem_raster=raw_dem, out_line3d_features=os.path.join(folder, '{}.gdb'.format(gdb), 'Drain3D'))
        ArcHydroTools.Smooth3DLine(in_line3d_features=os.path.join(folder, '{}.gdb'.format(gdb), 'Drain3D'), out_smoothline3d_features=os.path.join(folder, '{}.gdb'.format(gdb), 'SmoothDrain3D_UTM'))
        ...
        gp.CreateFeatureclass_management(os.path.join(folder, '{}.gdb'.format(gdb)), shape_out, 'POINT', '', 'DISABLED', 'DISABLED', epsg)
        ...
        while line_3D:
            ...
            RDEt = (feat_vertex[0][2] - feat_vertex[-1][2]) / max(0.0001, math.log(extract_length(line_3D, geometry_types)))
            point_x = -1
            point_y = -1
            comp_segment = 0
            ext_source = 0
            ...
            while v < len(feat_vertex):
                ...
                if v == len(feat_vertex) - 1:
                    ...
                    comp_segment += calc_horizontal_distance(feat_vertex[v][0], feat_vertex[v][1], feat_vertex[v - 1][0], feat_vertex[v - 1][1])
                    ext_source += calc_horizontal_distance(feat_vertex[v][0], feat_vertex[v][1], feat_vertex[v - 1][0], feat_vertex[v - 1][1])
            line_3D = cursor_rde.Next()
...
def main(env):
    ...
    knickpoints_extract(raw_dem=dem_path, shape_out=knick_name, drain_network=drainage_line_path, folder=folder, eq=equidistant,  gdb=gdb_name, epsg=epsg)
